...
AuthModelView::AuthModelView(AuthModel *model, QObject *parent)
    ...
    connect(
        m_model, &AuthModel::loginApiFinished, this,
        &AuthModelView::onApiFinished
    );
    connect(
        m_model, &AuthModel::loginApiError, this, &AuthModelView::onApiError
    );

    connect(
        m_model, &AuthModel::createAccountApiFinished, this,
        &AuthModelView::onApiFinished
    );
    connect(
        m_model, &AuthModel::createAccountApiError, this,
        &AuthModelView::onApiError
    );
}
...
void AuthModelView::createAccount(
    const QString &name,
    const QString &email,
