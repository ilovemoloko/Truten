...
void Database::changeSlotInfo(const std::string& id, const std::string& start_time, const std::string& end_time, int capacity) {
    pqxx::work txn(conn);
    txn.exec_params("UPDATE slots SET capacity = $1, start_time = $2, end_time = $3 WHERE ID = $4", capacity, start_time, end_time, id);
    txn.commit();
}
...
bool Database::addUserHours(const std::string &id, int amount) {
    try {
        pqxx::work txn(conn);
        ...
        std::cerr << "Error when trying to add hours to " << id << ": " << err.what() << "\n";
        return false;
    }
}

bool Database::banUser(const std::string &id, int duration) {
    try {
        ...
        std::cerr << "Error trying to ban " << id << " " << err.what() << "\n";
        return false;
    }
}
...
bool Database::isAdmin(const std::string &id) {
    try {
        ...
        std::cerr << "Error trying to determine if user is admin?? wtf?? " << err.what() << "\n";
        return false;
    }
}
...
bool Database::userExists(const std::string& id) {
    try {
        ...
        std::cerr << "Error trying to determine if user EXISTS?? wtf?? " << err.what() << "\n";
        return false;
    }
}
